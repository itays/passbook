...
export class TreeComponent implements OnInit, AfterViewInit  {
  ...
  onAddNewCat() {
    const dialogData = new MdDialogConfig();
    dialogData.data = {
      categories: this.categories
    };
    dialogData.disableClose = true;
    const dialogRef = this.dialog.open(AppAddCategoryDialog, dialogData);
    ...
    <div *ngIf="!isOnProgress">
      ...
      <form #newCategoryForm="ngForm">  
        <div md-dialog-content>
          <md-input-container>
            <input mdInput placeholder="New Category" name="newpass" 
            [(ngModel)]="newPass" #newpass="ngModel" (keyup)="checkExists()"required />
          </md-input-container>
        </div>
        <div md-dialog-actions>
          <button type="button" md-raised-button color="primary" (click)="onAddCategory()" 
          [disabled]="!newCategoryForm.form.valid || isExists">Add</button>
          <button type="button" md-raised-button (click)="dialogRef.close(false)">Cancel</button>
        </div>
      </form>
    </div>`,
})
export class AppAddCategoryDialog {
  ...
  constructor(public dialogRef: MdDialogRef<AppAddCategoryDialog>, private ps: PasswordsService) {
